...
# coverage <- request("https://codecov.io/api/gh/easystats") %>%
...
coverage <- data.frame(
  ...
  badge = coverage_status_badge("easystats", repo_summary$repo)
)
...
repo_summary <- left_join(repo_summary, downloads, by = c("repo" = "package")) %>% 
  ...
  mutate(Badge = ifelse(`Main branch` == "master", gsub("main", "master", Badge), Badge))
...
issue_progress_table <- issue_data2 %>%
  ...
  separate(col = "maintainer", into = c("maintainer", "maintainer_email"), sep = " <") %>% 
  mutate(
    maintainer_email = gsub(">", "", maintainer_email)
  ) %>% 
  # group_by(type, status) %>%
  arrange(event) %>%
  select(package, issue, maintainer, maintainer_email, event, status, type) %>%
  na.omit()

names(issue_progress_table) <- tools::toTitleCase(names(issue_progress_table))
  
reactable(
  issue_progress_table[, -4], 
  columns = list(
    Maintainer = colDef(cell = function(value, index) {
      url <- paste0("mailto: ", issue_progress_table[index, "Maintainer_email"])
      htmltools::tags$a(href = url, as.character(value))
    })
  ),
  groupBy = c("Package", "Type"),
  defaultPageSize = 11
)
```
...
summary_table <- repo_summary %>%
  ...
  select(package, maintainer, watchers, reverse_deps, weekly_downloads, open_issues, prs, p1, everything(), -description) %>%
  rename(!!p1 := p1)
```
...
library(httr2)
req <- request("https://codecov.io/api/gh/easystats") %>%
  req_headers("Authorization" = Sys.getenv("CODECOV_TOKEN"))
req
