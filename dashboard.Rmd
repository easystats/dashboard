...
cran_checks <- function(pkg) {
  ...
  paste(
    ...
    n_warnings, if (n_warnings > 1) "Warnings<br>" else "Warning<br>",
    n_errors, if (n_errors > 1) "Errors<br>" else "Error<br>"
  )
}
...
repo_summary <- left_join(repo_summary, downloads, by = c("repo" = "package")) %>% 
  ...
  select(repo, default_branch, last_published_cran, watchers, open_issues, prs, everything()) %>%
