...
cran_checks <- function(pkg) {
  ...
  if (all(check_status == "OK")) return("OK")
  ...
  if (n_notes > 0) {
    color_note <- "<span style=\"color: blue\">"
  } else {
    color_note <- "<span style=\"color: black\">"
  }

  if (n_warnings > 0) {
    color_warning <- paste0("<span style=\"color: ", gh_colors$red, "\">")
  } else {
    color_warning <- "<span style=\"color: black\">"
  }

  if (n_errors > 0) {
    color_error <- paste0("<span style=\"color: ", gh_colors$red, "\">")
  } else {
    color_error <- "<span style=\"color: black\">"
  }

  cran_page <- paste0("&nbsp;<a href=\"", url, "\">&#x1f517;</a>")
  # "<em><a href=\"", url, "\">&lt;&#x1f517;&nbsp;webpage&gt;</a></em><br>"

  paste0(
    color_note, n_notes, " Note", if (n_notes > 1) "s" else "", if (n_notes > 0) cran_page else "", "</span><br>",
    color_warning, n_warnings, " Warning", if (n_warnings > 1) "s" else "", if (n_warnings > 0) cran_page else "", "</span><br>",
    color_error, n_errors, " Error", if (n_errors > 1) "s" else "", if (n_errors > 0) cran_page else "", "</span><br>"
  )
}
...
prs <- pr_data %>% 
  filter(is.na(closed)) %>% 
  mutate(n_comments = unlist(map(comments, nrow)))

prs_2 <- list()
for (i in main_repos) {
  prs_2[[i]] <- gh::gh("/repos/:repo/pulls?state=OPEN", repo = paste0("easystats/", i)) %>% 
    tibble(
      repo = i,
      number = map_int(., "number"),
      title = map_chr(., "title"),
      url = map_chr(., "html_url"),
      draft = as.logical(map_chr(., "draft"))
    ) %>% 
    select(-1)
}
prs_2 <- do.call(rbind, prs_2)

  
pr_table <- prs %>%
  left_join(prs_2, by = c("repo", "issue" = "number")) %>% 
  mutate(
    title = paste0("<a href='", url, "' target = '_blank'>", title, "</a>"),
    draft = ifelse(draft, "Yes", "No")
  ) %>% 
  select(repo, issue, title, author, created, updated, n_comments, draft) %>% 
  rename(
    Repository = repo,
    PR = issue,
    Title = title,
