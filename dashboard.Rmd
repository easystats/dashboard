...
```{r workflow_status}
workflow_status <- readRDS("badge_url_data.rds")

reactable(
  workflow_status,
  columns = list(
    bayestestR = colDef(html = TRUE),
    correlation = colDef(html = TRUE),
    datawizard = colDef(html = TRUE),
    easystats = colDef(html = TRUE),
    effectsize = colDef(html = TRUE),
    insight = colDef(html = TRUE),
    modelbased = colDef(html = TRUE),
    parameters = colDef(html = TRUE),
    performance = colDef(html = TRUE),
    report = colDef(html = TRUE),
    see = colDef(html = TRUE)
  ),
  static = TRUE
)
```
