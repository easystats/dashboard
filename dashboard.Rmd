...
cran_checks <- function(pkg) {
  ...
  if (n_notes > 0) {
    text_note <- paste0(
      "<span style=\"color: blue\">", n_notes, " Note", if (n_notes > 1) "s" else "", "</span>"
    )
  } else {
    text_note <- NULL
  }
  ...
  if (n_warnings > 0) {
    text_warning <- paste0(
      "<span style=\"color: orange\">", n_warnings, " Warning", if (n_warnings > 1) "s" else "", "</span>"
    )
  } else {
    text_warning <- NULL
  }
  ...
  if (n_errors > 0) {
    text_error <- paste0(
      "<span style=\"color: red\">", n_errors, " Error", if (n_errors > 1) "s" else "", "</span>"
    )
  } else {
    text_error <- NULL
  }

  out <- paste0(c(text_note, text_warning, text_error, collapse = ", ")
  out <- paste0(out, "&nbsp;<a href=\"", url, "\">&#x1f517;</a>")
  # paste0(out, "<em><a href=\"", url, "\">&lt;webpage&gt;&#x1f517;</a></em><br>")
